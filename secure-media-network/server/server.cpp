...
int main() {
    ...
    const socket_handle_t server_socket = create_server_socket(config::kServerPort, config::kServerBacklog);
    if (server_socket == kInvalidSocket) {
        std::cerr << "[SERVER] Failed to create server socket." << std::endl;
        return 1;
    }

    std::cout << "[SERVER] Listening on " << config::kServerHost << ":" << config::kServerPort << "..." << std::endl;
    ...
    try {
        ...
        const std::string frame = decrypt_data(packet.encrypted_payload, config::kDemoAes256Key, iv_raw);
